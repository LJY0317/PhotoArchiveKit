...
- native duplicate-review GUI의 sidebar를 단일 검색 목록으로 단순화하고 Live Photo는 system `livephoto` symbol로 표시하며 그룹별 전체 사본/occurrence 수를 보여준다. detail은 standalone 사본 또는 Live Photo occurrence 하나당 비교 열 하나를 만들고 still+paired-video completeness, exact byte/SHA-256, filesystem/catalog facts, root/provenance/role, capture/Live Photo metadata, location history와 서로 다른 값/날짜 earliest/latest를 강조한다. preview부터 마지막 metadata row와 row 사이의 빈 공간까지 열 전체를 하나의 cleanup toggle surface로 확장하고 선택 테두리는 중첩 없이 열 전체에 하나만 그리며, 같은 열을 다시 클릭하면 정리 표시를 취소한다. 마지막 남은 사본을 클릭해도 control을 비활성화하지 않고 cleanup 선택을 그 사본으로 이동한다. Live Photo occurrence는 atomic resource set으로 유지하되 유일한 complete pair도 정리 후보로 선택할 수 있고 `유일한 페어`라는 사실을 명시한다. native AppKit help tag는 app-local 1초 initial hover delay를 사용한다. `EXACT`도 Live Photo에서는 `EXACT STILL`/`VIDEO`/`PAIR`/`RESOURCE`로 scope를 구분한다. 그룹 승인, registered-root 범위 filter, batch `검토 결과 제출`을 지원하며 제출 bundle은 opaque ID만 저장하고 media를 수정하지 않는다. 비교 위치 메뉴에서는 active registered root를 선택하고 새 폴더를 native macOS picker로 추가한 뒤 usage role/provenance를 지정해 현재 선택 위치들과 한 session으로 incremental scan할 수 있다.
