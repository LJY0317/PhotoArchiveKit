...
- duplicate-review의 SwiftUI 양축 scroll view에서 실제 AppKit `NSScrollView.hasHorizontalScroller`를 overflow에 맞춰 명시적으로 활성화하고, 열 이동 control은 scroll content가 아닌 viewport에 고정해 입력 장치와 overlay-scroller 정책에 관계없이 가로 탐색 affordance를 유지한다.
- horizontal overflow가 있는 duplicate-review에서는 macOS scroll indicator를 표시·flash하고 indicator를 하단 action bar 위로 inset한다. 세로휠만 있는 마우스도 가로 scrollbar drag 또는 항상 보이는 `열 이동` 좌/우 control로 한 열씩 이동할 수 있다.
