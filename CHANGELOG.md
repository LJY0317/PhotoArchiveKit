...
- duplicate-review metadata 표의 시간 정보를 `촬영 · 메타데이터 시각`과 `파일 · 라이브러리 시각`으로 묶고, 사진 원본에서 ImageIO로 EXIF `DateTimeOriginal`/`DateTimeDigitized`, TIFF `DateTime`, original/digitized offset·subsecond를 local-only 교차검증 근거로 표시한다. 여러 원본 timestamp가 초 단위로 일치하는지도 별도 상태로 보여준다.
- duplicate-review metadata 표의 시간 정보를 원본 촬영 시각 → 파일·유입·관측 시각 순으로 묶고 상단에 배치한다. 대표 촬영시각의 timestamp·UTC offset·source·confidence를 함께 보여주고, ImageIO에서 EXIF `DateTimeOriginal`/`DateTimeDigitized`, 각 offset/subsecond, TIFF `DateTime`을 별도 원본 근거로 읽어 교차검증한다. Live Photo/video의 QuickTime creation date도 같은 촬영 섹션에서 비교하며, filesystem timestamp는 별도 의미로 명확히 구분한다.
- duplicate-review GUI의 `검토 결과 제출` 중간 단계를 사용자 주 workflow에서 제거하고 `정리 전 확인` → current session/resource/root/path/Live Photo atomicity 및 가능한 fresh SHA-256 재검증 → 제품 설정의 macOS Trash/custom quarantine 이동으로 직접 연결했다. retained exact counterpart가 없는 명시적 선택은 staging/primary-library에서만 별도 경고와 함께 reversible move를 허용하며, opaque decision bundle은 cleanup 직전 local audit snapshot으로 자동 저장한다. visible app bundle도 `PhotoArchiveKit.app` 하나로 통일한다.
- native duplicate-review GUI의 sidebar를 단일 검색 목록으로 단순화하고 Live Photo는 system `livephoto` symbol로 표시하며 그룹별 전체 사본/occurrence 수를 보여준다. detail은 standalone 사본 또는 Live Photo occurrence 하나당 비교 열 하나를 만들고 still+paired-video completeness, exact byte/SHA-256, filesystem/catalog facts, root/provenance/role, capture/Live Photo metadata, location history와 서로 다른 값/날짜 earliest/latest를 강조한다. 하나의 SwiftUI Grid가 열 너비와 row 높이를 함께 소유하고 실제 first/last cell anchor로 연속 선택 테두리를 그려 preview부터 마지막 metadata row의 빈 공간까지 시각적 열 경계와 cleanup hit-area를 일치시킨다. 기본 열은 희미한 neutral boundary, hover는 system accent, 실제 cleanup 선택만 red border/tint를 사용하고 green은 추천 keeper/complete Live Photo 같은 semantic evidence에 제한한다. 같은 열을 다시 클릭하면 정리 표시를 취소하고, 마지막 남은 사본까지 정리 대상으로 누르면 자동 selection swap 대신 전체 정리 confirmation을 요구한다. Live Photo occurrence는 atomic resource set으로 유지하되 유일한 complete pair도 명시적 전체 정리 확인을 거쳐 정리 후보로 선택할 수 있고 `유일한 페어`라는 사실을 명시한다. sidebar는 하단 action bar만큼 scroll inset을 확보해 마지막 row가 가려지지 않게 한다. native AppKit help tag는 app-local 1초 initial hover delay를 사용한다. `EXACT`도 Live Photo에서는 `EXACT STILL`/`VIDEO`/`PAIR`/`RESOURCE`로 scope를 구분한다. 그룹 승인, registered-root 범위 filter, batch `검토 결과 제출`을 지원하며 제출 bundle은 opaque ID만 저장하고 media를 수정하지 않는다. 비교 위치 메뉴에서는 active registered root를 선택하고 새 폴더를 native macOS picker로 추가한 뒤 usage role/provenance를 지정해 현재 선택 위치들과 한 session으로 incremental scan할 수 있다. toolbar에는 catalog presentation만 다시 읽는 단일 refresh만 남기고 실제 registered-root 재scan은 비교 위치 메뉴 안에서 명시적으로 실행한다.
