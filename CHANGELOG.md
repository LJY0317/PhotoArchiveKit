...
- duplicate-review metadata 표에서 원본 촬영시각을 filesystem timestamp와 분리한다. `촬영 시각 (원본 메타데이터)`을 Date Added/파일시스템 시각보다 먼저 표시하고 timestamp·UTC offset·source·confidence를 한 행에 요약하며, 기존 `파일 생성 시각`/`현재 수정 시각`은 `파일시스템 생성 시각`/`파일시스템 수정 시각`으로 명확히 표시한다.
