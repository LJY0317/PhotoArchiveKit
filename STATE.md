...
- canonical exact keeper v3: archive/reference root는 protected replica로 automatic removal에서 제외한다. primary `local_library` 안의 standalone exact duplicate는 trusted capture evidence와 shallow path를 이용해 keeper 하나를 고를 수 있고, 같은 primary root의 exact Live Photo duplicate occurrence는 complete status를 우선해 whole occurrence 단위로 keeper/candidate를 나눈다. import source는 retained exact coverage/source semantics 조건에서만 cleanup한다. cross-root backup replica를 전역 1-copy로 축소하지 않는다.
...
- incremental exact-hash cache: root별 cache evidence를 SQLite에서 bulk-load한 뒤 unchanged resource는 same path + byte size + mtime(+ available filesystem ID)으로 기존 SHA-256을 재사용하고, 같은 volume의 manual move/rename은 filesystem ID로 path 변경 뒤에도 hash를 재사용. archive root는 local cache miss 뒤 matching stable marker의 portable inventory hash도 seed로 사용할 수 있음. `archive-index --fresh`는 local/portable cache를 모두 무시하고 media byte 전체를 다시 읽음. cache는 performance accelerator일 뿐 mutation authority가 아니며 quarantine/archive-copy 등 mutation boundary는 계속 fresh SHA-256을 요구
...
- `duplicate-review`: current exact reconciliation의 AUTO item을 local-private Finder workspace로 materialize한다. 기본값은 active root registry와 일치하는 가장 최근 complete scan snapshot을 SQLite에서 재구성해 전체 media 재scan 없이 review를 즉시 만들고, `--refresh`에서만 fresh scan한다. group별 `KEEPER`/`CANDIDATE` symbolic link와 `locations.txt`만 만들며 media bytes는 copy/move/rename/delete하지 않는다. `--candidate-root`로 한 root의 cleanup candidate만 좁힐 수 있고 agent-safe report에는 workspace/media path를 노출하지 않는다. 향후 scan은 Live Photo timed-metadata status를 resource별로 catalog에 보존하고, 이전 catalog snapshot은 exact-byte evidence가 하나의 video variant로 수렴할 때만 complete status를 보수적으로 재구성한다.
...
- incremental exact-hash cache는 local SQLite + archive-root portable inventory까지 구현됐다. metadata probe(EXIF/QuickTime/Live Photo timed metadata) 자체는 아직 unchanged-file cache를 재사용하지 않으므로 실제 HDD benchmark에서 metadata probe가 병목으로 측정될 때만 별도 최적화를 연다.
...
5. native incremental exact-hash cache는 local SQLite + removable archive inventory까지 구현하고 synthetic/CLI로 검증했다. 실제 HDD read-only repeat scan에서는 cached exact hash 46개가 재사용됐지만 metadata probe 자체는 여전히 4,195 resource 전부 수행했다. 다음 성능 작업은 필요하면 explicit `archive-index --fresh`와 incremental repeat를 비교해 metadata probe cache가 실제 병목인지 측정한 뒤에만 연다. Czkawka exact accelerator는 별도 측정 이점이 생길 때만 다시 평가한다.
...
7. 사용자 지정 HDD의 **깊은 사진 최상위 root**에 대한 read-only scan은 완료했다. 아직 marker/inventory는 쓰지 않았다. 다음 핵심 단계는 Mac local library와 이 HDD root를 함께 비교해 exact overlap, HDD에 이미 보관된 logical asset, Mac에만 남은 asset을 agent-safe count로 계산하는 것이다. marker 또는 inventory write는 별도 사용자 승인 후에만 수행하고 media copy는 하지 않는다.
