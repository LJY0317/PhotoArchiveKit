...
- `photoarchive quarantine`: 기본 dry-run, `--apply`에서 strong `automatic_redundant` exact 후보를 사용자 지정 local quarantine으로 이동. preference-sensitive keeper 선택은 기본 제외하며, 사람이 검토한 현재 plan item ID를 반복 가능한 `--approve-item`으로 명시한 경우에만 mutation authority를 얻는다. apply 직전 source/preferred의 regular-file·size·symlink boundary와 fresh SHA-256을 재검증하고, Live Photo item은 전체 resource가 검증된 뒤 이동하며, session 실패 시 이미 이동한 resource를 전체 rollback. 완료 session에는 local restore manifest를 남김
...
- real cross-staging targeted cleanup validation: 사용자가 명시적으로 선택한 standalone exact group 3개에 대해 각 keeper 1개와 redundant candidate 2개를 다시 확인했다. current PhotoArchiveKit quarantine dry-run이 정확히 6 candidate resource를 fresh preflight했고, 전체 AUTO set을 잘못 적용하지 않기 위해 CLI에 item-selection filter가 없는 현재 상태에서는 별도 targeted maintenance quarantine으로 이 6개만 원래 staging 위치에서 이동했다. 각 move 직전 candidate와 keeper의 full SHA-256을 다시 계산해 일치를 검증했지만 raw hash 문자열은 agent output/manifest에 기록하지 않았다. postcondition은 keeper 3/3 존재, source candidate 6/6 부재, quarantine destination 6/6 keeper와 exact 일치, audit manifest complete이며, 즉시 post-move two-staging replan에서 해당 3 group은 더 이상 reconciliation item으로 나타나지 않았다. 이후 사용자의 local development policy에 맞춰 해당 reversible session을 개인 경로가 repository에 남지 않는 local-private temporary-deletion target 아래로 relocation하고 manifest destination도 재기록했으며, 비어 버린 이전 temporary container는 제거했다. `AGENTS.md`는 향후 임시 제거 전 local-private policy file을 읽도록 요구한다.
