...
- native `photoarchive-review` SwiftUI 1차 GUI를 추가했다. `PhotoArchiveCore`의 최신 reusable active-root scan report와 current reconciliation plan을 presentation model로 변환하고, sidebar에서 전체/standalone/Live Photo 필터와 파일명·위치 검색, detail에서 keeper/candidate Quick Look thumbnail·경로·크기·Finder Date Added·capture evidence·keeper rationale를 비교한다. read-only milestone이라 Finder reveal 외 media mutation control은 없고 cached UI state는 mutation authority가 아니다. Apple Photos의 duplicate grouping과 Gemini 2의 sidebar/grid review 패턴을 참고하되 standard macOS split view, toolbar, SF Symbols를 사용한다.
