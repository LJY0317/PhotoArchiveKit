...
6. preferred/canonical representation의 immutable archive plan과 staging copy/apply executor를 구현했다. 실제 working catalog 전체 preflight는 marker 전 `AUTO 3,319 item / 4,947 resource`, `REVIEW 4,859 item / 5,727 resource`였고, full plan에서 logical AUTO 10 item만 파생한 `15 resource` bounded batch를 실제 외장 HDD에 copy/verify/catalog/snapshot한 뒤 replay no-op까지 검증했다. 이후 Takeout 3개 root에 stable marker를 초기화했고, SHA-256 chunk autorelease로 post-marker full-plan memory blocker를 해결했다. 현재 실제 persisted plan과 archive-copy dry-run 기준은 `AUTO 7,283 item / 9,098 resource`, `REVIEW 895 item / 1,576 resource`, dry-run copy-required `9,098`이다. 다음 단계는 전체 9,098 resource를 한 번에 apply하지 말고 logical item 단위의 더 큰 bounded batch로 확대해 HDD postcondition과 resume 특성을 계속 검증하는 것이다.
...
12. HDD archive destination plan과 staging copy -> byte verify -> verified finalization -> destination scan/catalog commit -> resumable manifest/snapshot 경로는 synthetic/CLI, 실제 외장 HDD 1-resource smoke, real-library 전체 dry-run, 10-item/15-resource bounded apply, post-marker 9,098-resource persisted full plan/dry-run까지 완료. 다음은 bounded apply를 단계적으로 확장하고, 그 다음 user-installed rclone replica/check adapter와 최소 1개의 독립 verified replica를 연결
