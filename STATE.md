...
8. archive same-root dedupe 정책을 real HDD에서 read-only 검증했다. HDD archive 하나만 새 scan/plan한 결과 standalone exact group 9개가 각 1 resource의 same-root candidate를 만들었고 Live Photo candidate/review는 0이었다. 모든 candidate와 preferred survivor가 동일 archive root 안에 있어 cross-root replica collapse는 발생하지 않았다. 이어 `quarantine` dry-run을 실행했을 때 9개 모두 current keeper preference가 strong mutation authority에 못 미쳐 실제 quarantine 후보는 0으로 거부됐다. 이는 exact group 자체는 관리 대상으로 찾되 deterministic/weak survivor 선택만으로 media를 옮기지 않는 기존 안전 정책과 일치한다. HDD media는 수정하지 않았다.
