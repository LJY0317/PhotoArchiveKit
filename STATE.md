...
- archive-copy executor는 synthetic/temporary filesystem에서 staging -> byte verify -> final verify -> destination scan/catalog commit -> archive-local snapshot까지 검증됐지만 실제 외장 HDD real-library apply는 아직 수행하지 않았다. 독립 replica verification/rclone adapter, permanent delete, cloud upload도 아직 없다. `organize`는 same-session deterministic camera-name rename/flatten 전용이며 general-purpose move command가 아니다.
...
6. preferred/canonical representation의 immutable archive plan과 staging copy/apply executor를 구현했다. 실제 working catalog 전체 preflight 결과는 `AUTO 3,319 item / 4,947 resource`, `REVIEW 4,859 item / 5,727 resource`였고 review 중 `3,964` item은 marker 없는 Takeout source가 원인이었다. 전체 apply는 보류하고 full plan에서 logical AUTO 10 item만 별도 local-private batch plan으로 파생해 `15 resource`를 실제 외장 HDD에 copy/verify/catalog/snapshot한 뒤 즉시 replay no-op까지 검증했다. 다음 단계는 Takeout marker 정책을 결정하거나 현재 local-authoritative AUTO를 더 큰 bounded batch로 확장하는 것이다.
...
12. HDD archive destination plan과 staging copy -> byte verify -> verified finalization -> destination scan/catalog commit -> resumable manifest/snapshot 경로는 synthetic/CLI, 실제 외장 HDD 1-resource smoke, real-library 전체 dry-run, 10-item/15-resource bounded apply까지 완료. 전체 `4,947` AUTO resource를 한 번에 적용하지 않고 marker 정책과 batch 크기를 먼저 정한 뒤, user-installed rclone replica/check adapter와 최소 1개의 독립 verified replica를 연결
