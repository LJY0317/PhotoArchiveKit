...
`scan`, `plan`, `organize-plan`은 media에 대해 read-only다. `archive-plan`은 media-read-only지만 local-private persisted plan 파일을 생성한다. `archive-copy`는 기본 dry-run이며 `--apply`에서 archive destination에 검증된 copy와 hidden operation/catalog state를 추가하지만 source media는 수정하지 않는다. `quarantine`과 `organize`도 기본 dry-run이고 명시적 `--apply`에서만 제한된 AUTO item을 이동한다. 영구 삭제는 없다.
...
- archive-copy executor는 synthetic/temporary filesystem, 실제 외장 HDD 1-resource smoke, real-library 10-item/15-resource bounded apply까지 검증됐지만 전체 real-library apply는 아직 수행하지 않았다. Takeout marker 초기화 뒤 전체 immutable `archive-plan` 재생성은 두 번 각각 `exit 137`로 강제 종료됐고 plan 파일이 완성되지 않았다. 동일 4,151 marker-gated resource를 별도 streaming SHA-256 verifier로 전수 확인했을 때는 모두 통과했으므로 media/source evidence 문제가 아니라 full scan+plan 경로의 resource/runtime 안정성 문제를 별도 blocker로 본다. 독립 replica verification/rclone adapter, permanent delete, cloud upload도 아직 없다. `organize`는 same-session deterministic camera-name rename/flatten 전용이며 general-purpose move command가 아니다.
...
6. preferred/canonical representation의 immutable archive plan과 staging copy/apply executor를 구현했다. 실제 working catalog 전체 preflight는 marker 전 `AUTO 3,319 item / 4,947 resource`, `REVIEW 4,859 item / 5,727 resource`였고, full plan에서 logical AUTO 10 item만 파생한 `15 resource` bounded batch를 실제 외장 HDD에 copy/verify/catalog/snapshot한 뒤 replay no-op까지 검증했다. 이후 Takeout 3개 root에 stable marker를 초기화하고 marker-gated 4,151 resource를 fresh SHA-256 전수 검증해 3,964 item 전부 AUTO 승격 조건을 만족함을 확인했다. 예상 post-marker 상태는 `AUTO 7,283 item / 9,098 resource`, `REVIEW 895 item / 1,576 resource`다. 다만 post-marker full immutable plan 생성이 두 번 `exit 137`로 종료됐으므로, 다음 단계는 이 full scan+plan 안정성 blocker를 해결하고 실제 새 plan을 정상 생성하는 것이다. 그 전에는 추가 대량 apply를 하지 않는다.
...
12. HDD archive destination plan과 staging copy -> byte verify -> verified finalization -> destination scan/catalog commit -> resumable manifest/snapshot 경로는 synthetic/CLI, 실제 외장 HDD 1-resource smoke, real-library 전체 marker-before dry-run, 10-item/15-resource bounded apply까지 완료. Takeout marker 이후 fresh source evidence 기준 AUTO 후보는 9,098 resource로 늘었지만 full immutable plan 생성이 아직 안정적으로 완료되지 않는다. plan 생성 blocker를 해결한 뒤 bounded apply를 확장하고, 그 다음 user-installed rclone replica/check adapter와 최소 1개의 독립 verified replica를 연결
