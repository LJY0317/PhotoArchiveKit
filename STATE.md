...
- `organize`/`organize-plan --singleton-leaf-only`: 기존 camera-name/date organization 규칙과 safety gate를 그대로 유지하면서, 다른 filesystem entry가 전혀 없는 nested directory 하나에 계획된 logical asset 하나만 있는 경우로 scope를 좁힌다. REVIEW item을 AUTO로 승격하지 않는다.
