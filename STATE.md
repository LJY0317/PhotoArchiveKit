...
5. native incremental exact-hash cache는 local SQLite + removable archive inventory까지 구현하고 synthetic/CLI로 검증했다. 다음 성능 작업은 실제 HDD 첫 `archive-index --fresh`와 즉시 incremental repeat의 wall-clock/I/O/cache-hit 차이를 측정한 뒤 metadata probe cache가 실제 병목일 때만 연다. Czkawka exact accelerator는 별도 측정 이점이 생길 때만 다시 평가한다.
...
7. 다음 real-library 단계는 사용자가 실제로 관리하는 HDD의 **깊은 사진 최상위 root**를 지정한 뒤, 그 root에 marker가 없다면 별도 사용자 승인 후 marker만 초기화하고 `archive-index`를 우선 read-only로 실행하는 것이다. 첫 full index와 즉시 repeat의 시간/cache-hit을 측정하고 Mac local library와 HDD archive의 exact overlap/미보관 asset을 계산한다. inventory write(`archive-index --apply`)는 index 결과를 확인한 뒤에만 수행하며 media copy는 하지 않는다.
