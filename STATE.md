...
- `photoarchive quarantine`: 기본 dry-run, `--apply`에서 strong `automatic_redundant` exact 후보를 saved duplicate-cleanup destination으로 이동. product setting이 없으면 macOS Trash가 기본이며 `photoarchive settings deletion-destination quarantine PATH`로 custom app-managed quarantine을 선택할 수 있고 `--trash`/`--to PATH`는 one-shot override다. preference-sensitive keeper 선택은 기본 제외하며, 사람이 검토한 현재 plan item ID를 반복 가능한 `--approve-item`으로 명시한 경우에만 mutation authority를 얻는다. apply 직전 source/preferred의 regular-file·size·symlink boundary와 fresh SHA-256을 재검증하고, Live Photo item은 전체 resource가 검증된 뒤 이동하며, session 실패 시 이미 이동한 resource를 전체 rollback한다. custom quarantine만 local restore manifest를 남긴다. 모든 candidate 이동 성공 뒤 해당 operation으로 비게 된 source parent chain만 registered root 직전까지 deepest-first 제거한다.
...
- role-aware canonical exact keeper: 각 registered root에 `staging`, `primary_library`, `archive`, `import_source`, `reference` 중 하나의 user-changeable usage role을 저장한다. staging/primary/archive는 **같은 root 안의** exact duplicate를 그 root의 survivor 하나로 줄일 수 있고, standalone exact copy는 여러 `staging` root 사이에서도 하나의 staging survivor로 줄일 수 있다. cross-staging keeper는 usage-role retention과 copy-marker/name 규칙을 먼저 적용하고, 알아보기 쉬운 source filename을 opaque generated name보다 우선한 뒤 강한 근거가 동률일 때 양쪽에 Finder `Date Added`가 있으면 더 이른 사본을 우선한다. 같은 usage role 안의 provenance category 자체는 generic keeper 품질 순위에 사용하지 않는다. `Date Added`가 한쪽에 없으면 그 사실만으로 우열을 만들지 않는다. cross-root Live Photo는 이 규칙으로 자동 collapse하지 않으며 archive/primary는 다른 root의 replica 때문에 generic reconciliation에서 제거하지 않는다. import source는 same-root dedupe를 지원하되 Google Takeout은 source-folder semantics capture 뒤에만 허용하고, cross-root cleanup은 staging/primary/archive retained exact counterpart 또는 기존 source semantics 조건을 통과해야 한다. reference는 비교 전용 read-only라 dedupe/import cleanup authority가 없다. staging→archive offload cleanup은 별도 coverage workflow로 남긴다.
...
- native `photoarchive-review` SwiftUI 1차 GUI를 추가했다. `PhotoArchiveCore`의 최신 **재사용 가능한 exact-duplicate scan root subset**과 current reconciliation plan을 presentation model로 변환하므로, unrelated active root가 추가되거나 별도 1-root scan이 더 최신이어도 유효한 duplicate-review snapshot을 다시 열 수 있다. sidebar에서 전체/standalone/Live Photo 필터와 파일명·위치 검색, detail에서 keeper/candidate Quick Look thumbnail·경로·크기·Finder Date Added·capture evidence·keeper rationale를 비교하며 현재 snapshot에 포함된 root scope도 표시한다. read-only milestone이라 Finder reveal 외 media mutation control은 없고 cached UI state는 mutation authority가 아니다. Apple Photos의 duplicate grouping과 Gemini 2의 sidebar/grid review 패턴을 참고하되 standard macOS split view, toolbar, SF Symbols를 사용한다.
- raw `swift run` 실행이 LaunchServices의 정식 app bundle 의미를 갖지 않아 Mission Control activation이 어색할 수 있던 문제를 수정했다. GUI가 `NSApplication` regular activation policy를 명시하고, `scripts/build-review-app.sh`가 `io.photoarchivekit.review` foreground `.app` bundle을 조립하며 `scripts/run-review-app.sh`가 이를 `open`으로 실행한다. 실제 local launch에서 LaunchServices `ApplicationType=Foreground`, `backgroundOnly` 아님을 확인했다.
...
swift run photoarchive quarantine [--trash|--to PATH] [--apply] [options] ROOT...
...
swift run photoarchive settings deletion-destination quarantine PATH
...
- cross-staging Date Added/provenance validation: 두 disposable staging root의 standalone exact fixture에서 양쪽 `Date Added`를 local SQLite/current scan report에 보존하고, 더 강한 filename 근거가 같으면 먼저 추가된 staging copy를 keeper로 선택하면서 나중 copy는 AUTO quarantine preflight authority를 얻는 것을 확인했다. regression은 일부러 earlier root=`unknown`, later root=`local_library` provenance로 두어 provenance category가 같은-role generic keeper를 가로채지 않는 것도 고정했다. 별도 fixture에서는 더 늦게 추가된 `IMG_5199.JPG`가 더 이른 opaque UUID filename보다 우선해 filename evidence가 `Date Added`보다 앞서는 것도 확인했다. 실제 current `Downloads` + `Pictures` read-only plan에서는 `a.jpg` group이 `Pictures/a.jpg` 1개 keeper + `Pictures/a 2.jpg`/`Downloads/a.jpg` 2개 AUTO candidate, `output.mp4`도 같은 구조로 바뀌었고 `filesModified=false`였다. provenance는 Takeout/source semantics 같은 safety fact로 계속 보존하되 같은 usage role의 generic exact-copy quality rank에서는 제외한다. cross-root Live Photo와 primary/archive replica 정책은 변경하지 않았다.
- Finder review의 사람이 읽는 `comparison.txt`, workspace README, stale/offline 안내문을 한국어로 전환했다. exact-byte 동일, 실제 target 크기, 선택 근거, filesystem 생성/`Date Added`/수정 시각과 xattr 차이의 의미를 한국어로 설명하고, scan capture source가 `file_creation_date/fallback`인 경우 이를 embedded metadata 차이라고 잘못 표현하지 않고 **파일시스템 생성 시각 fallback이며 파일 내부 촬영 메타데이터가 아님**을 분리해 표시한다. 내부 folder/status token(`CURRENT`, `KEEPER`, `CANDIDATE`)은 도구 식별을 위해 괄호/이름에 유지한다.
- real cross-staging targeted cleanup validation: 사용자가 명시적으로 선택한 standalone exact group 3개에 대해 각 keeper 1개와 redundant candidate 2개를 다시 확인했다. current PhotoArchiveKit quarantine dry-run이 정확히 6 candidate resource를 fresh preflight했고, 전체 AUTO set을 잘못 적용하지 않기 위해 CLI에 item-selection filter가 없는 당시 상태에서는 별도 targeted maintenance quarantine으로 이 6개만 원래 staging 위치에서 이동했다. 각 move 직전 candidate와 keeper의 full SHA-256을 다시 계산해 일치를 검증했지만 raw hash 문자열은 agent output/manifest에 기록하지 않았다. postcondition은 keeper 3/3 존재, source candidate 6/6 부재, quarantine destination 6/6 keeper와 exact 일치였고 즉시 post-move two-staging replan에서 해당 3 group은 더 이상 reconciliation item으로 나타나지 않았다. 이후 사용자가 default deletion destination을 macOS Trash로 바꾸면서 동일 6개 quarantine resource를 keeper와 fresh exact 재검증한 뒤 `FileManager.trashItem`으로 Trash에 이동했고 해당 targeted session의 빈 directory chain 4개를 제거했다. 남아 있던 과거 custom temporary-trash root는 media 8,010개 전부에 대해 폴더 외부 current catalog counterpart 존재를 확인한 뒤 양쪽 full-file SHA-256을 fresh 재검증해 8,010/8,010 일치·실패 0을 확인하고 root 전체를 macOS Trash로 이동했다. 현재 duplicate cleanup destination은 product `settings.json`의 `system_trash`다.
- duplicate-cleanup destination/empty-parent validation: 설정 파일이 없는 synthetic Mac은 `system_trash` 기본값을 사용하고 custom quarantine path 저장/복원 후 다시 Trash로 전환되는 것을 self-test로 고정했다. system-Trash apply는 기존 quarantine의 role/fresh SHA-256/Live Photo atomicity verifier를 destination-independent하게 재사용하며 production에서는 `FileManager.trashItem`을 호출한다. self-test는 실제 Trash 대신 fake mover를 주입해 exact candidate 1개만 이동하고 keeper/root를 보존한 채 이번 move로 비어진 nested parent 2개만 제거하는 것을 검증했다. empty-parent cleaner는 `.DS_Store`, AppleDouble `._*`, Windows `Thumbs.db`/`ehthumbs.db`/`desktop.ini`, KDE `.directory` 등 명시된 regenerable metadata만 남은 directory도 effectively empty로 정리하며 unknown hidden file/symlink/package는 blocker로 유지한다.
...
7. 사용자 지정 HDD의 **깊은 사진 최상위 root** read-only scan과 current Mac↔HDD `archive-coverage` 비교를 완료했다. current exact overlap은 0이고 양쪽 Live Photo counterpart도 0이므로, 현 HDD는 current Mac의 replica가 아니라 별도 archive 집합으로 취급한다. current root policy는 Mac photo staging root=`staging`, Downloads=`staging`, HDD=`archive`, Takeout=`import_source`이며 active `reference` root는 없다. staging→archive offload cleanup은 실제 Finder copy/분류로 overlap이 생긴 뒤 `archive-index/coverage -> exact+Live Photo protection 검증 -> staging cleanup plan` 순서로 열며, 그 전에는 synthetic 데이터로 억지로 mutation workflow를 확대하지 않는다. 같은 Mac↔HDD coverage 비교는 실제 HDD 내용이 바뀐 뒤에만 다시 수행한다.
8. archive same-root dedupe 정책을 real HDD에서 끝까지 검증했다. HDD archive 하나만 새 scan/plan한 결과 standalone exact group 9개가 각 1 resource의 same-root candidate를 만들었고 Live Photo candidate/review는 0이었다. 모든 candidate와 preferred survivor가 동일 archive root 안에 있어 cross-root replica collapse는 발생하지 않았다. 처음에는 keeper preference가 strong mutation authority에 못 미쳐 quarantine이 거부됐고, 이후 사용자가 Finder duplicate-review의 9개 keeper/candidate를 직접 검토해 전부 승인했다. 반복 가능한 `quarantine --approve-item ITEM_ID`를 추가해 current automatic plan에 존재하는 preference-sensitive item만 명시 승인할 수 있게 했고, 승인 전 dry-run은 9 item / 9 resource, `--apply`는 fresh SHA-256과 기존 role/path/symlink safety gate를 다시 통과한 동일 9 resource만 reversible quarantine으로 이동했다. post-apply HDD archive plan은 AUTO 0 / REVIEW 0이며, 생성된 restore manifest의 dry-run도 9 item / 9 resource로 정상 복구 가능함을 확인했다. 이후 2026-09-07 사용자의 기존 명시 승인에 따라 PhotoArchiveKit 제품 기능이 아닌 외부 maintenance 경로에서 해당 quarantine destination 9개만 재검증 후 영구 제거했다. 삭제 직전 restore dry-run은 9/9 통과했고 regular-file/non-symlink/byte-size/source-absence/session-boundary를 다시 확인했으며, 삭제 후 destination 9/9 부재와 manifest 보존을 확인했다. PhotoArchiveKit 자체에는 permanent-delete 기능을 추가하지 않는다.
9. organization apply 전 persisted immutable plan/approval token은 향후 offline/replay mutation에 필요할 때 추가한다. same-session organize는 post-move catalog transaction까지 이미 완료됨
10. `cleanup-empty-dirs`는 real library apply와 postcondition까지 완료로 닫는다. 같은 organization manifest 기준 412 directory 제거 후 잔여 후보 0을 확인했다.
11. strict Live Photo timed-metadata validation은 구현과 real-library 검증까지 완료로 닫는다. 현재 library의 기존 complete occurrence 1,824개가 모두 통과했고 reconciliation `AUTO 0 / REVIEW 227`도 유지됐다.
12. versioned sanitized JSONL catalog export/restore는 synthetic round-trip, stable opaque ID rebind, Takeout collection semantics preservation까지 완료. archive-copy도 verified destination scan 뒤 archive `.photoarchive/catalog`에 snapshot을 배치하도록 구현했다. 실제 HDD snapshot/restore 확인은 real-library archive apply에서 수행
13. HDD archive-copy transaction 경로는 synthetic/CLI, 실제 외장 HDD 제한 smoke, persisted full plan/dry-run까지 충분히 검증했으며 현재 사용자 workflow에서는 더 이상 자동 copy를 확대하지 않는다. user-managed HDD index/coverage workflow가 안정된 뒤 독립 verified replica가 필요할 때 user-installed rclone replica/check adapter를 연결한다.
14. user-managed archive의 existing folder hierarchy capture는 구현 완료. 이 folder를 신규 event의 자동 분류 example로 학습하는 event-level archive-guided classifier는 현재 index/coverage workflow가 real HDD에서 검증된 뒤로 유지
15. North Star archive workflow가 real library에서 안정화되기 전에는 Google upload와 broader provider convenience를 보류
