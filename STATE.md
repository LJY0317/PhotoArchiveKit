...
- cross-staging Date Added validation: 두 disposable staging root의 standalone exact fixture에서 양쪽 `Date Added`를 local SQLite/current scan report에 보존하고, 더 강한 filename 근거가 같으면 먼저 추가된 staging copy를 keeper로 선택하면서 나중 copy는 AUTO quarantine preflight authority를 얻는 것을 확인했다. 별도 fixture에서는 더 늦게 추가된 `IMG_5199.JPG`가 더 이른 opaque UUID filename보다 우선해 filename evidence가 `Date Added`보다 앞서는 것도 확인했다. 실제 current `Downloads` + `Pictures` read-only plan에서는 `a.jpg` group이 `Pictures/a.jpg` 1개 keeper + `Pictures/a 2.jpg`/`Downloads/a.jpg` 2개 AUTO candidate, `output.mp4`도 같은 구조로 바뀌었고 `filesModified=false`였다. cross-root Live Photo와 primary/archive replica 정책은 변경하지 않았다.
