...
현재 native local duplicate-review GUI는 이 판단의 private 정보를 사람이 바로 볼 수 있게 합니다. duplicate group 하나씩 physical copy의 위치, keeper/candidate 역할, Quick Look 미리보기와 filter/search를 제공하며 **로컬 전용**으로 동작합니다. agent-safe report에는 계속 opaque group/root ID와 count만 전달합니다. 1차 milestone은 read-only이고 duplicate 판단은 `PhotoArchiveCore`에 그대로 위임해 별도 판단 엔진으로 분기하지 않습니다.
