...
Finder workspace보다 빠르게 사람이 검토하려면 native SwiftUI duplicate-review 앱을 사용할 수 있습니다. 앱은 최근 중복 검사 결과를 열고, 실제 사본 또는 Live Photo 한 세트당 비교 열 하나를 만들어 같은 metadata 행에 맞춰 보여줍니다. sidebar와 미리보기는 소비자용 문구를 사용하며 내부 planner ID, keeper/candidate 같은 구현 용어, `EXACT STILL` 같은 debug 배지는 표시하지 않습니다. PhotoArchiveKit이 남길 사본을 추천할 때는 카드 안에 **남기기 추천**과 함께 `사진과 비디오가 함께 있는 Live Photo입니다`, `Mac에 더 먼저 추가된 사본입니다`처럼 짧고 이해하기 쉬운 이유를 보여줍니다. 완전한 Live Photo는 단순히 **Live Photo**라고 표시합니다. metadata 표는 Live Photo 그룹의 `[still]` / `[video]` 슬롯을 모든 사본에서 같은 순서로 유지하고, 없는 구성은 `—`로 표시해 좌우 비교가 어긋나지 않게 합니다. 열을 클릭하면 해당 사본이 **삭제 예정**으로 표시되고 다시 클릭하면 취소되며, 두 사본 중 반대쪽 열을 누르면 삭제 선택이 그쪽으로 이동합니다. 모든 사본을 삭제 대상으로 고르는 동작만 미리보기의 **삭제 대상으로 선택** 버튼과 별도 확인을 요구합니다. 삭제 예정 사본이 하나라도 있으면 바로 **휴지통으로 이동…** 단계로 갈 수 있고, 확인창에는 사본/파일/용량 요약과 실제로 주의가 필요한 경고만 보여줍니다. 경로·hash·Live Photo 구성 같은 안전 검사는 사용자에게 구현 세부사항을 늘어놓지 않고 실제 이동 직전에 내부적으로 다시 수행합니다. `비교 위치` 메뉴에서는 비교할 폴더를 고르거나 새 폴더를 추가할 수 있으며, toolbar 새로고침은 media를 다시 읽지 않고 최근 결과만 다시 불러옵니다. 일반 GUI 실행은 raw SwiftPM executable보다 아래 launcher를 사용해 정식 `PhotoArchiveKit.app` bundle로 엽니다.
