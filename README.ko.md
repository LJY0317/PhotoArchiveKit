...
Finder workspace보다 빠른 사람 검토가 필요하면 native SwiftUI duplicate-review 앱을 사용할 수 있습니다. 1차 GUI는 현재 active root 전체를 한 번에 다시 scan하도록 강제하지 않고 최신 **재사용 가능한 exact-duplicate root subset**의 AUTO group을 엽니다. sidebar는 별도 타입 탭 없이 하나의 검색 가능한 목록으로 유지하고 Live Photo group만 목록 안에서 표시합니다. detail은 **standalone 물리 사본 또는 Live Photo occurrence 하나당 한 열**을 만들고 모든 사본을 같은 metadata 행에 맞춰 세로로 비교합니다. local-private 표에는 path, 정확한 byte 수와 SHA-256, filesystem 정보, 최초 파일명, root/provenance/role, 촬영 근거, Live Photo metadata 상태, location history를 폭넓게 표시하고 값이 다르면 `다름`, 날짜 계열은 `가장 빠름`/`가장 늦음`을 표시합니다. 이 상세 정보는 로컬 GUI에만 머물며 agent-safe interface를 넓히지 않습니다. 새 duplicate 판단 엔진을 만들지 않고 `PhotoArchiveCore`의 current reconciliation 결과를 그대로 표시하며 아직 mutation authority를 갖지 않습니다. 일반 GUI 실행은 raw SwiftPM executable인 `swift run photoarchive-review`보다 아래 launcher를 사용합니다. launcher는 실제 macOS `.app` bundle을 만든 뒤 LaunchServices로 열어 Dock, Mission Control, app activation이 정상적인 foreground application 의미를 갖게 합니다.
