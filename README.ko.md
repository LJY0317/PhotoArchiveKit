...
Finder workspace보다 빠른 사람 검토가 필요하면 native SwiftUI duplicate-review 앱을 사용할 수 있습니다. 현재 active root 전체를 한 번에 다시 scan하도록 강제하지 않고 최신 **재사용 가능한 exact-duplicate root subset**의 AUTO group을 엽니다. sidebar는 별도 타입 탭 없이 하나의 검색 목록으로 유지하고 Live Photo는 Apple의 system `livephoto` symbol로 표시하며 각 그룹의 전체 사본/occurrence 수를 함께 보여줍니다. detail은 **standalone 물리 사본 또는 Live Photo occurrence 하나당 한 열**을 만들고 모든 사본을 같은 metadata 행에 맞춰 세로로 비교하며, still+paired-video가 모두 있는 온전한 occurrence를 더 강하게 표시합니다. 하나의 SwiftUI Grid가 copy-column 너비와 metadata-row 높이를 함께 소유하고, 연속 선택 테두리는 Grid가 실제로 배치한 첫 cell과 마지막 cell의 좌표를 측정해 그리므로 preview부터 마지막 metadata row의 빈 공간까지 보이는 열 경계와 cleanup hit-area가 일치합니다. 기본 남김 열은 아주 희미한 중립 경계만 사용하고 hover 때만 system accent를 약하게 보여주며, 명시적인 cleanup 선택에만 빨간 테두리/tint를 사용합니다. 초록색은 추천 keeper나 complete Live Photo pair 같은 semantic evidence에만 제한합니다. 열 어디를 클릭해도 그 사본/occurrence 전체가 정리 후보로 toggle되고 같은 열을 다시 클릭하면 취소됩니다. 마지막 남은 사본까지 정리 대상으로 누르면 자동으로 다른 사본과 선택을 반전하지 않고, 그룹 전체를 정리 대상으로 표시할지 명시적인 확인을 요청합니다. `이 사본 남기기`는 다른 사본들을 한 번에 정리 표시하는 빠른 보조 동작으로 유지합니다. Live Photo occurrence는 still+paired-video 원자 단위로 계속 처리하지만, 유일한 complete pair도 정리 후보로 선택할 수 있으며 GUI가 `유일한 페어`라는 사실과 전체 정리 확인에서의 경고를 명시합니다. native AppKit help tag의 초기 hover 지연은 이 앱에서만 1초로 사용합니다. exact evidence도 일반 파일의 `EXACT`와 Live Photo의 `EXACT STILL`/`EXACT VIDEO`/`EXACT PAIR`/`EXACT RESOURCE`를 구분해 resource byte identity와 occurrence completeness를 혼동하지 않게 합니다. 그룹별 `검토 완료`를 누른 뒤 하단 `정리 전 확인`에서 선택한 그룹·사본·파일 수, 용량, reversible destination과 위험 경고를 확인하고 실제 cleanup을 실행할 수 있습니다. apply 직전에는 session/item/resource, root/path/regular-file 상태, Live Photo occurrence atomicity, 가능한 catalog hash와 retained exact counterpart를 다시 검증합니다. 기본 destination은 제품 설정의 macOS 휴지통이고 custom quarantine 설정도 같은 product setting을 따릅니다. retained exact counterpart가 없는 명시적 선택은 staging/primary-library root에서만 허용하고 별도 경고를 표시합니다. opaque decision bundle은 사용자가 별도로 `제출`하는 단계가 아니라 실제 cleanup 직전 local audit snapshot으로 자동 저장됩니다. toolbar의 `비교 위치`는 active registered root 전체를 보여주며, macOS 기본 폴더 선택기로 새 폴더를 고른 뒤 usage role과 provenance를 명시적으로 지정해 등록할 수 있습니다. 실제 registered-root 파일을 다시 비교하려면 이 메뉴의 `선택 위치 다시 스캔`을 사용하고, toolbar의 단일 `검토 화면 새로고침`은 디스크를 다시 scan하지 않고 최신 catalog presentation만 읽습니다. 등록 후에는 현재 선택된 기존 root들과 새 root를 한 scan session으로 incremental 비교하고, 현재 snapshot 바깥의 기존 registered root도 선택해 함께 다시 scan할 수 있습니다. 일반 GUI 실행은 raw SwiftPM executable인 `swift run photoarchive-review`보다 아래 launcher를 사용합니다. launcher는 하나의 정식 `PhotoArchiveKit.app` bundle을 만든 뒤 LaunchServices로 열어 Dock, Mission Control, app activation이 정상적인 foreground application 의미를 갖게 합니다.
