...
Finder workspace보다 빠른 사람 검토가 필요하면 native SwiftUI duplicate-review 앱을 사용할 수 있습니다. 1차 GUI는 최신 reusable catalog snapshot의 AUTO exact group을 sidebar에서 필터/검색하고, keeper와 candidate의 Quick Look 미리보기, 파일명·위치·Date Added·keeper rationale를 한 화면에서 비교하는 **read-only** surface입니다. 새 duplicate 판단 엔진을 만들지 않고 `PhotoArchiveCore`의 current reconciliation 결과를 그대로 표시하며 아직 mutation authority를 갖지 않습니다. 일반 GUI 실행은 raw SwiftPM executable인 `swift run photoarchive-review`보다 아래 launcher를 사용합니다. launcher는 실제 macOS `.app` bundle을 만든 뒤 LaunchServices로 열어 Dock, Mission Control, app activation이 정상적인 foreground application 의미를 갖게 합니다.
