...
media를 복사하지 않고 사람이 Finder에서 AUTO exact 결정을 검토하려면 `duplicate-review`로 symbolic link만 들어 있는 local-private review workspace를 만들 수 있습니다. 기본 동작은 active root registry와 일치하는 가장 최근 complete scan snapshot을 재사용하므로 이미 계산한 review를 다시 열기 위해 전체 media library를 다시 읽지 않습니다. 대신 그 결정에 실제로 등장하는 파일의 현재 size, modification time, filesystem identity와 가능한 경우 stable root marker identity를 빠르게 확인합니다. 그대로인 group은 `CURRENT` + `KEEPER/CANDIDATE`, 달라진 group은 `STALE` + `OLD_KEEPER/OLD_CANDIDATE`, 필요한 root가 연결되지 않았으면 `OFFLINE`으로 명확히 표시합니다. 각 group의 local-private `comparison.txt`와 안내문은 한국어로 표시하며 exact-byte evidence, Finder symbolic-link 크기와 구분된 실제 원본 target size, embedded/capture metadata, filesystem 생성/`Date Added`/수정 시각, 파일명/경로 차이, filesystem identity, extended-attribute 동일 여부, keeper 선택 이유를 기록합니다. `file_creation_date` fallback은 embedded capture metadata와 분리해서 "파일시스템 생성 시각을 대신 사용한 값"임을 명시합니다. 또한 절대적인 "모든 metadata가 동일"이라고 과장하지 않고 "PhotoArchiveKit이 현재 검사하는 metadata 범위에서 차이를 찾지 못했다"고 범위를 명시합니다. `--preference-only`는 complete Live Photo 유지, 명시적인 `copy`/`복사본` 파일명 표식, 실제 사용자 검토로 승인된 알아보기 쉬운 출처형 파일명, 강한 근거가 동률일 때 더 이른 Finder `Date Added`, 같은 root의 더 얕은 경로처럼 강한 기본 결정을 숨깁니다. 숫자형 복사 suffix는 상대 파일명과 실제로 대응될 때만 강한 근거로 취급합니다. 파일 basename과 같은 부모 폴더에 들어 있는 사본은 상대가 `무제 폴더`/`Untitled Folder`/`새 폴더`/`New Folder` 같은 명백한 placeholder 폴더일 때 강하게 우선합니다. 의미 있는 우열 근거가 없는 deterministic tie-break는 명시적인 사용자 승인이 있기 전까지 review 전용이며 자동 quarantine 권한을 갖지 않습니다. `--candidate-root`로 한 root의 후보만 좁힐 수 있고, `--refresh`는 current root를 incremental rescan하며 `--refresh --fresh`는 명시적으로 metadata/hash 전체 재계산을 강제합니다. review 자체는 원본 media를 move/rename/delete/copy하지 않습니다.

Finder workspace보다 빠르게 사람이 검토하려면 native SwiftUI duplicate-review 앱을 사용할 수 있습니다. 앱은 최근 중복 검사 결과를 열고, 실제 사본 또는 Live Photo 한 세트당 비교 열 하나를 만들어 같은 metadata 행에 맞춰 보여줍니다. sidebar와 미리보기는 소비자용 문구를 사용하며 내부 planner ID, keeper/candidate 같은 구현 용어, `EXACT STILL` 같은 debug 배지는 표시하지 않습니다. 각 비교 카드는 사진을 가장 먼저 보여주고, 완전한 Live Photo는 썸네일 안에 작고 절제된 native `livephoto` 배지로 표시합니다. 삭제 선택은 별도 `삭제 예정` 문구 대신 빨간 열 상태와 썸네일 우상단의 작은 빨간 휴지통 배지로 표현하고, 선택을 되돌리는 버튼은 destructive 색을 쓰지 않는 **선택 취소**로 표시합니다. PhotoArchiveKit이 남길 사본을 추천할 때는 사진 아래에 **남기기 추천**과 함께 `사진과 비디오가 함께 있는 Live Photo입니다`, `Mac에 더 먼저 추가된 사본입니다`처럼 짧고 이해하기 쉬운 이유를 보여줍니다. 기본 metadata 표는 소비자가 실제 비교에 쓰는 **파일명, 전체 경로, 미디어 종류, 촬영 시각, Finder에 추가된 시각, 파일 생성 시각, 파일 수정 시각, 전체 크기**만 남깁니다. `촬영 시각`은 사본마다 사람이 읽기 좋은 최종값 하나만 표시하며 EXIF `DateTimeOriginal` 또는 QuickTime creation date처럼 실제 촬영 metadata가 있을 때만 사용합니다. Google Takeout 시간이나 파일 생성 시각 같은 fallback만 있으면 `—`로 둡니다. Finder 추가 시각·파일 생성 시각·파일 수정 시각은 촬영 시각 바로 아래에 연속 배치하고, Live Photo에서는 `[still]` / `[video]` 슬롯을 같은 순서로 유지해 누락된 구성은 `—`로 표시합니다. `전체 크기`도 한 행 안에서 Live Photo의 `[still]` / `[video]` 현재 크기를 따로 보여주며 `2,868,570 bytes`처럼 반올림하지 않은 전체 정수 byte 값을 사용합니다. 기본 행 아래의 **고급 정보 보기**를 누르면 이름/형식, 원본 EXIF·QuickTime 촬영 근거, PhotoArchiveKit 관측 기록, 위치/출처, 크기/저장공간, hash/Live Photo 무결성, 파일시스템 권한, 내부 진단 정보를 성격별로 묶은 전체 비교 행이 펼쳐지고 버튼은 **고급 정보 숨기기**로 바뀝니다. 펼친 내용의 맨 아래에도 **고급 정보 숨기기**를 한 번 더 제공하므로 끝까지 읽은 뒤 바로 접을 수 있습니다. SwiftUI 내부 상태로 표 높이가 늘거나 줄어도 native scroll document 크기를 즉시 다시 측정해 고급 정보의 마지막 행까지 내려가거나 다시 최상단으로 올라갈 수 있습니다. 마지막 기본 행 아래에는 별도 separator를 두지 않아 카드의 위아래 마감을 맞춥니다. 열을 클릭하면 해당 사본이 삭제 대상으로 선택되고 다시 클릭하면 취소되며, 두 사본 중 반대쪽 열을 누르면 삭제 선택이 그쪽으로 이동합니다. 모든 사본을 삭제 대상으로 고르는 동작만 미리보기의 **삭제 대상으로 선택** 버튼과 별도 확인을 요구합니다. 삭제 대상으로 고른 사본이 하나라도 있으면 바로 **휴지통으로 이동…** 단계로 갈 수 있고, 확인창에는 사본/파일/용량 요약과 실제로 주의가 필요한 경고만 보여줍니다. 경로·hash·Live Photo 구성 같은 안전 검사는 사용자에게 구현 세부사항을 늘어놓지 않고 실제 이동 직전에 내부적으로 다시 수행합니다. `비교 위치` 메뉴에서는 비교할 폴더를 고르거나 새 폴더를 추가할 수 있으며, toolbar 새로고침은 media를 다시 읽지 않고 최근 결과만 다시 불러옵니다. 일반 GUI 실행은 raw SwiftPM executable보다 아래 launcher를 사용해 정식 `PhotoArchiveKit.app` bundle로 엽니다.

고급 정보의 `이름과 형식`, `촬영 메타데이터`, `PhotoArchiveKit 기록`, `위치와 출처`, `크기와 저장 공간`, `무결성과 Live Photo`, `파일 시스템`, `내부 기록`은 첫 데이터 행의 높이를 늘리는 라벨이 아니라 독립된 section header row로 표시합니다. 따라서 각 section의 첫 실제 값 행도 다른 metadata 행과 같은 수직 간격을 사용합니다. 고급 정보 disclosure는 큰 비교 Grid 자체를 애니메이션하지 않으며, 기본 비교 열의 선택/hover outline도 기본 metadata 범위에 고정해 펼침·접힘 때 기존 본문의 재합성을 최소화합니다.

고급 정보 펼침/접힘은 큰 비교표 전체에 애니메이션을 적용하지 않고 즉시 전환하며, 기존 viewport와 document view를 유지한 채 고급 행만 추가하거나 제거합니다. document 높이만 바뀌는 경우에는 AppKit scroll view 전체를 강제로 retile하지 않아 본문 전체가 한 번 깜빡이는 현상을 피합니다.

비교 영역은 SwiftUI Grid를 유지하면서 직접 소유하는 AppKit 스크롤뷰에 배치하고 표준 스크롤 막대를 항상 표시합니다. 하단 가로 막대를 드래그하거나 **Shift + 마우스 휠**로 좌우 이동할 수 있으며 일반 휠은 세로로 이동합니다. 넓은 창에서는 열 너비 상한을 유지하고 비교 영역 전체를 가운데 배치해 양옆 여백을 균형 있게 확보합니다. 하단 action bar는 split view 아래의 별도 레이아웃 행을 차지하므로 가로 스크롤 막대를 가리지 않습니다. 실행 스크립트는 기본적으로 최적화된 release 앱을 만들며, 디버깅할 때는 `PHOTOARCHIVE_BUILD_CONFIGURATION=debug`를 지정합니다.

GUI 언어는 우선 한국어 문구의 완성도를 높이는 데 집중합니다. 아직 실제 두 번째 번역이 없는 동안에는 작동하지 않는 언어 전환 control을 노출하지 않습니다. 영어 번역을 시작할 때는 String Catalog 기반 localization을 추가하고, 그 시점에 macOS의 앱별 언어 설정과 일관되는 `설정 > 언어` 선택 UI를 붙이는 방향으로 확장합니다.
...
현재 native local duplicate-review GUI는 이 판단의 private 정보를 사람이 바로 볼 수 있게 합니다. duplicate group 하나씩 physical copy/Live Photo occurrence를 비교 열로 정렬하고 Quick Look 미리보기, local-private catalog/filesystem 정보와 차이 표시를 제공합니다. **로컬 전용**으로 동작하며 agent-safe report에는 계속 opaque group/root ID와 count만 전달합니다. 1차 milestone은 read-only이고 duplicate 판단은 `PhotoArchiveCore`에 그대로 위임해 별도 판단 엔진으로 분기하지 않습니다.
