...
Finder workspace보다 빠른 사람 검토가 필요하면 native SwiftUI duplicate-review 앱을 사용할 수 있습니다. 현재 active root 전체를 한 번에 다시 scan하도록 강제하지 않고 최신 **재사용 가능한 exact-duplicate root subset**의 AUTO group을 엽니다. sidebar는 별도 타입 탭 없이 하나의 검색 목록으로 유지하고 Live Photo는 Apple의 system `livephoto` symbol로 표시합니다. detail은 **standalone 물리 사본 또는 Live Photo occurrence 하나당 한 열**을 만들고 모든 사본을 같은 metadata 행에 맞춰 세로로 비교하며, still+paired-video가 모두 있는 온전한 occurrence를 더 강하게 표시합니다. preview나 명시적인 버튼을 눌러 정리 대상을 표시하거나 남길 occurrence를 정하고, 그룹별 검토 완료를 누른 뒤 여러 그룹을 한 번에 `검토 제출`할 수 있습니다. toolbar의 `비교 위치`에서는 현재 snapshot의 registered root를 비교 범위로 켜고 끌 수 있습니다. 제출 결과는 session/item/subject/resource의 opaque ID만 담은 local decision bundle로 저장되고 **파일을 이동하지 않습니다**. 실제 cleanup은 여전히 별도 fresh verification 단계입니다. 일반 GUI 실행은 raw SwiftPM executable인 `swift run photoarchive-review`보다 아래 launcher를 사용합니다. launcher는 실제 macOS `.app` bundle을 만든 뒤 LaunchServices로 열어 Dock, Mission Control, app activation이 정상적인 foreground application 의미를 갖게 합니다.
