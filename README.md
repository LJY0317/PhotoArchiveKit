...
- reuse exact SHA-256 evidence for unchanged files from the Mac-local SQLite catalog, and for archive roots optionally seed that cache from the root's hidden `.photoarchive/inventory-v1.jsonl`; scan-style commands accept `--fresh` to bypass metadata/hash reuse, while `archive-index --fresh` bypasses its local/portable hash caches and re-reads every media resource;
