...
For faster human review, the native SwiftUI duplicate-review app opens the latest reusable **exact-duplicate root subset** instead of requiring one cached scan to match every currently active root. Its first milestone is intentionally read-only: one unified searchable sidebar marks Live Photo groups in place, while the detail view uses a single vertical flow for keeper/candidate previews and expandable metadata such as paths, media/component roles, Finder Date Added, capture-time evidence, keeper rationale, and the snapshot's root scope. It does not invent a second duplicate engine or grant mutation authority to cached UI state. For normal GUI use, launch it with the script below rather than the raw SwiftPM `swift run photoarchive-review` executable. The launcher assembles a real macOS `.app` bundle and opens it through LaunchServices so Dock, Mission Control, and app activation use normal foreground-application semantics.
...
The native local duplicate-review GUI exposes the private side of this decision directly: one duplicate group at a time, with physical copies, locations, keeper/candidate roles, Quick Look previews, and filters/search. This is deliberately a **local-only** view; agent-safe reports continue to expose opaque group/root IDs and counts without filenames or paths. The current first milestone remains read-only and delegates all duplicate reasoning to `PhotoArchiveCore` rather than becoming a second decision engine.
