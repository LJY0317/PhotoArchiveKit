...
For faster human review, the native SwiftUI duplicate-review app opens the latest reusable **exact-duplicate root subset** instead of requiring one cached scan to match every currently active root. A unified searchable sidebar uses the system `livephoto` symbol for Live Photo groups and shows the total copy/occurrence count for each group. The detail view uses **one comparison column per physical standalone copy or Live Photo occurrence** and keeps all copies aligned across vertically scrolling metadata rows; complete still+paired-video occurrences are emphasized, and different/earliest/latest values are called out. A single SwiftUI Grid owns copy-column widths and metadata-row heights so the visible column boundary and cleanup hit area match from preview through otherwise-empty space in the final metadata rows. The continuous selection border is measured from the Grid's actual first and last cells, so it follows the same geometry as the clickable column. Unmarked copies use only a faint neutral boundary, hover uses the system accent color, and red border/tint is reserved for explicit cleanup selection; green remains limited to semantic evidence such as the recommended keeper or a complete Live Photo pair. Clicking anywhere in the column toggles the whole copy/occurrence for cleanup, and clicking a marked column again cancels the mark. If the user tries to mark the final remaining copy, the app does not silently invert the selection: it asks for explicit confirmation before all copies are marked for cleanup. Explicit `Keep This Copy` remains available for quickly marking all peers instead. A Live Photo occurrence is still atomic, but even the only complete still+paired-video occurrence may be marked for cleanup; the GUI explicitly labels it as the only complete pair and includes that fact in the remove-all confirmation. Native AppKit help tags use a one-second initial hover delay for this app only. The exact-evidence badge distinguishes plain file `EXACT` from Live Photo `EXACT STILL`, `EXACT VIDEO`, `EXACT PAIR`, or `EXACT RESOURCE` so resource identity is not confused with occurrence completeness. After marking groups reviewed, **Review Cleanup** opens a final confirmation with selected groups/copies/files, total size, reversible destination, and risk warnings. Immediately before apply, PhotoArchiveKit revalidates the scan/item/resource IDs, root/path/regular-file state, Live Photo occurrence atomicity, available catalog hashes, and retained exact counterparts. The default destination is the product setting for macOS Trash; a user-configured custom quarantine remains supported by the same setting. Explicit resources with no retained exact counterpart are allowed only from staging/primary-library roots and are clearly warned. The opaque decision bundle is saved automatically as a local audit snapshot immediately before cleanup rather than exposed as a required user `submit` step. The **Comparison Locations** menu lists active registered roots; a new folder can be chosen with the native macOS folder picker, assigned an explicit usage role and provenance, registered, and then incrementally scanned together with the currently selected roots in one comparison session. Use `Rescan Selected Locations` in that menu when registered-root files should be read again; the toolbar's single `Refresh Review` only reloads the latest catalog presentation and does not rescan media. Existing registered roots outside the current snapshot can also be selected and rescanned together. For normal GUI use, launch it with the script below rather than the raw SwiftPM `swift run photoarchive-review` executable. The launcher assembles the single visible `PhotoArchiveKit.app` bundle and opens it through LaunchServices so Dock, Mission Control, and app activation use normal foreground-application semantics.
