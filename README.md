...
For faster human review, the native SwiftUI duplicate-review app opens the latest reusable **exact-duplicate root subset** instead of requiring one cached scan to match every currently active root. Its first milestone is intentionally read-only: a macOS sidebar supports filtering/search, while the detail view compares keeper and candidate Quick Look previews, names, locations, Finder Date Added, keeper rationale, and the snapshot's root scope without inventing a second duplicate engine or granting mutation authority to cached UI state. For normal GUI use, launch it with the script below rather than the raw SwiftPM `swift run photoarchive-review` executable. The launcher assembles a real macOS `.app` bundle and opens it through LaunchServices so Dock, Mission Control, and app activation use normal foreground-application semantics.
