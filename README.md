...
For faster human review, the native SwiftUI duplicate-review app opens the latest reusable **exact-duplicate root subset** instead of requiring one cached scan to match every currently active root. Its first milestone is intentionally read-only: one unified searchable sidebar marks Live Photo groups in place, while the detail view uses **one comparison column per physical standalone copy or Live Photo occurrence** and keeps all copies aligned across vertically scrolling metadata rows. The local-private table can show paths, exact byte counts and SHA-256, filesystem facts, original names, root/provenance/role state, capture evidence, Live Photo metadata state, location history, and visible difference/earliest/latest markers. These details remain local-only and do not expand the agent-safe interface. For normal GUI use, launch it with the script below rather than the raw SwiftPM `swift run photoarchive-review` executable. The launcher assembles a real macOS `.app` bundle and opens it through LaunchServices so Dock, Mission Control, and app activation use normal foreground-application semantics.
