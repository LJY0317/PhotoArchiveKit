...
GUI는 실제 cleanup 직전에 local decision bundle을 자동 저장한다. 이 bundle은 schema/session/item/subject/resource의 opaque ID와 시각만 포함하고 filename/path/hash/metadata dump를 저장하지 않는다. `photoarchive review-decisions --agent-json`은 필요할 때 같은 opaque decision을 진단/audit 용도로 읽을 수 있다. GUI cleanup의 실행 권한은 이 bundle 자체에서 나오지 않으며, current plan/session/resource와 Live Photo atomicity, local current-file evidence를 다시 검증한 뒤 reversible destination으로만 이동한다.
